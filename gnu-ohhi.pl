...
% A little faster.  Since we are not using them on CLPB variables anyway.
no_dups_labeled([]).
...
% gprolog gets command-line input from argument_value/2. Look it up in the manual.
% Use in tandem with shell expansion.
main :-
